...
import type { DataDepartment, Department } from 'src/ts/interfaces/models/department';
...
const save = async () => {
  ...
  if (check.status) {
    const response = await createDepartment(dataDepartment.name, dataDepartment.parent ?? null);
