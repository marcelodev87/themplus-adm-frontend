...
import { useOrderStore } from 'src/stores/coupon-store';
...
const customFilterOrder = (
  ...
  return rows.filter((item) => {
    return (
      (item.counter.name && item.counter.name.toLowerCase().includes(searchTerm)) ||
      (item.counter.email && item.counter.email.toLowerCase().includes(searchTerm)) ||
      (item.counter.cpf && item.counter.cpf.toLowerCase().includes(searchTerm)) ||
      (item.counter.cnpj && item.counter.cnpj.toLowerCase().includes(searchTerm))
    );
  });
};
